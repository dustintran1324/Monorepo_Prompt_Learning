...
import { PromptLearningState, AttemptData } from '../types';
...
export function PromptLearningProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const savedUserId = loadFromLocalStorage('promptLearning_userId');
    const savedAttempts = loadFromLocalStorage('promptLearning_attempts');
    const savedCurrentAttempt = loadFromLocalStorage('promptLearning_currentAttempt');
